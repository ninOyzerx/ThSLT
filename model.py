...
#input_shape=(30,1662) >> 30 คือเฟรมรูป 1662 คือจำนวน keypoint (จุดบนมือ+หน้า+ท่าทาง)
...
# Train model
# epochs=500 คือจำนวนรอบในการรันโมเดล
# เทรนเพื่อให้ค่า loss ลดลง (น้อยกว่า 0) และให้ค่า accuracy เพิ่มขึ้น (เข้าใกล้ 1)
# เมื่อรันเสร็จจะสร้างโฟลเดอร์ Logs ขึ้นมา
# ไม่จำเป้นต้องเทรนทุกรอบก็ได้
model.fit(X_train, y_train, epochs=600, batch_size=200, callbacks=[tb_callback])
print()
model.summary()
...
plt.imshow(confus, cmap='Blues', interpolation='nearest')
plt.title('Confusion Matrix')
plt.colorbar()
classes = actions  # Replace with your class labels if needed
...
fmt = 'd'
thresh = confus.max() / 2.
for i in range(confus.shape[0]):
    for j in range(confus.shape[1]):
        plt.text(j, i, format(confus[i, j], fmt),
                 ha="center", va="center",
                 color="white" if confus[i, j] > thresh else "black")
